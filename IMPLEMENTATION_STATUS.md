...
| P4.3 | Wire breaker into `fetch_with_retry` (skip provider when open) | done | P4.2, P1.4 | agent-p4.3-2026-05-15 | proxy/p2-5-p2-6-p4-3-p3-3-p6-1-combined | 2026-05-15 | https://github.com/medfata/sourcemaxai/pull/10 |
...
- 2026-05-15 | agent-p7-4-2026-05-15 | P7.4 = ops only (no code). Cutover sequence + rollback procedure captured in `PROXY_CUTOVER_RUNBOOK.md`. Env example files NOT changed: `USE_PROXY_POOL=false` is still the right local-dev default; the flip is per-deployment in Railway. P7.4 marked `done` only after the user reports 48h prod observation completed cleanly. | Spec §7 step 4 requires a real-world soak before P7.5 can rip the direct path; documenting the procedure is the implementable artifact for this task. |
- 2026-05-15 | user-2026-05-15 | P7.4 soak waived; mark done immediately. P7.5 cleared to ship without canary/cutover observation. | App not yet launched, no real users to protect. Staged rollout gates protect prod traffic; with zero traffic the soak adds no signal. Runbook (`PROXY_CUTOVER_RUNBOOK.md`) kept on disk for future re-deployments or scale-up. |
- 2026-05-15 | agent-p7-5-2026-05-15 | P7.5 deletes shadow + canary scaffolding (`_emit_shadow_event`, `_owner_in_canary_bucket`, `CANARY_BUCKET_COUNT`, `proxy_canary_active` log) AND the `USE_PROXY_POOL` / `PROXY_POOL_SHADOW` / `PROXY_POOL_CANARY` flags from `RuntimeConfig` + 3 env example files. Also deletes `PROXY_CUTOVER_RUNBOOK.md` because every flag it referenced is gone. KEEPS `pool is None → _list_transcripts_direct` as a **dev-only** fallback because prod config validation already rejects boot without IPRoyal creds (`config.py:171`), so `pool is None` is unreachable in prod by construction; in dev it's the only way to fetch a transcript without paid proxy creds. Inline comment captures the WHY. | Plan §7 step 5 says "remove direct-fetch path" — interpreted as removing it from the prod code path. Deleting the dev fallback would break local pipeline testing without proxy creds for no production benefit. Stale runbook = harmful per CLAUDE.md "no backwards-compat shims"; if a future re-rollout needs staging, recreate from this decision-log entry + git history. |
