...
- 2026-05-15 | agent-p7-5-2026-05-15 | P7.5 deletes shadow + canary scaffolding (`_emit_shadow_event`, `_owner_in_canary_bucket`, `CANARY_BUCKET_COUNT`, `PROXY_POOL_SHADOW`, `PROXY_POOL_CANARY` config + env entries) and the production direct-fetch branch. KEEPS `pool is None → _list_transcripts_direct` as a **dev-only** fallback because prod config validation already rejects boot without IPRoyal creds (`config.py:171`), so `pool is None` is unreachable in prod by construction; in dev it's the only way to fetch a transcript without paid proxy creds. Inline comment captures the WHY. | Plan §7 step 5 says "remove direct-fetch path" — interpreted as removing it from the prod code path. Deleting the dev fallback would break local pipeline testing without proxy creds for no production benefit. Decision recorded so future readers don't re-add shadow/canary thinking they're missing. |
